...
class DrawView_jump : View {
    ...
    private var dataset_hipY = mutableListOf<Double>()
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        dataset_hipY.add(0.5*(mDrawPoint[8].y+mDrawPoint[11].y).toDouble())
        ...
        if (dataset_Height.size > 11) {
            for (i in 10..dataset_Height.size-1) {
                ...
                for (i in 10..dataset_Height.size-1) {
                    if (dataset_hipY[i] != 0.0) {
                        temp = Math.min(dataset_hipY[i], temp)
                        //temp = dataset_ankleY[i]
                        jumpratio =
                            (dataset_hipY[mark]  - temp) / (parameter_height/(1-0.039))
        ...
        canvas.drawText(
            ...
            for ((index, pointF) in mDrawPoint.withIndex()) {
                ...
                if (index == 0||index == 8||index == 11|| index == 10||index ==13){
