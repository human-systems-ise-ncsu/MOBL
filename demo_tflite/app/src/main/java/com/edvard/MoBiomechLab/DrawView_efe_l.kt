...
class DrawView_efe_l : View {
    ...
    private var dataset_lelbow = mutableListOf<Double>()
    private var dataset_lelbow_av= mutableListOf<Double>()
    ...
    fun setDrawPoint(
        ...
        val L_lowerarmX = (mDrawPoint[4].x - mDrawPoint[3].x).toDouble()
        val L_upperarmX = (mDrawPoint[2].x - mDrawPoint[3].x).toDouble()
        val L_lowerarmY = (mDrawPoint[4].y - mDrawPoint[3].y).toDouble()
        val L_upperarmY = (mDrawPoint[2].y - mDrawPoint[3].y).toDouble()
        val L_elbow_angle_cos =
            ((L_lowerarmX * L_upperarmX + L_lowerarmY * L_upperarmY) / (Math.sqrt(
                Math.pow(
                    L_lowerarmX,
                    2.0
                ) + Math.pow(L_lowerarmY, 2.0)
            ) * Math.sqrt(Math.pow(L_upperarmX, 2.0) + Math.pow(L_upperarmY, 2.0))))
        val L_elbow_angle = (180 * Math.acos(L_elbow_angle_cos) / Math.PI)
        temp.add(L_elbow_angle)
        if (temp.size < 3){
            dataset_lelbow.add(L_elbow_angle)
        }
        if (temp.size >= 5)  {
            dataset_lelbow.add((temp [temp.size - 1] + temp[temp.size - 2] + temp[temp.size - 3] + temp[temp.size - 4] + temp[temp.size - 5])/5)
        }
        time.add(SystemClock.uptimeMillis())
        if (dataset_lelbow.size >= 2)  {
            val a = (dataset_lelbow [dataset_lelbow.size - 1] - dataset_lelbow[dataset_lelbow.size - 2]) / (time[time.size -1] - time[time.size - 2])
            dataset_lelbow_av.add(a*1000)
        }
        if(temp.size >= 101){
            temp.clear()
            dataset_lelbow.clear()
            dataset_lelbow_av.clear()
            time.clear()
        }
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawText(
            "Left elbow angle ="+String.format("%.2f",(180 * Math.acos(L_elbow_angle) / Math.PI))+" degs",
            300F,
            150F,
            mPaint2
        )
        if (dataset_lelbow_av.size >=1) {
            canvas.drawText(
                "Left elbow angular velocity ="+String.format("%.2f",dataset_lelbow_av[dataset_lelbow_av.size-1]) + " degs/s", 280F, 450F,
                mPaint2
            )
        }
        val paint = Paint()
        ...
        dataset_lelbow.forEachIndexed { index, currentDataPoint ->

            if (index < dataset_lelbow.size - 1) {
                val nextDataPoint = dataset_lelbow[index + 1]
        ...
        dataset_lelbow_av.forEachIndexed { index, currentDataPoint ->

            if (index < dataset_lelbow_av.size - 1) {
                    val nextDataPoint = dataset_lelbow_av[index + 1]
                    val startX = index.toFloat()+ XPoint
