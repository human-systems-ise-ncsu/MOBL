...
class DrawView_saa_r : View {
    ...
    private var dataset_rshoulder = mutableListOf<Double>()
    private var dataset_rshoulder_av= mutableListOf<Double>()
    ...
    fun setDrawPoint(
        ...
        val R_trunkX = (mDrawPoint[11].x - mDrawPoint[5].x).toDouble()
        val R_upperarmX = (mDrawPoint[6].x - mDrawPoint[5].x).toDouble()
        val R_trunkY = (mDrawPoint[11].y - mDrawPoint[5].y).toDouble()
        val R_upperarmY = (mDrawPoint[6].y - mDrawPoint[5].y).toDouble()
        val R_shoulder_angle_cos =
            ((R_trunkX * R_upperarmX + R_trunkY * R_upperarmY) / (Math.sqrt(
                Math.pow(
                    R_trunkX,
                    2.0
                ) + Math.pow(R_trunkY, 2.0)
            ) * Math.sqrt(Math.pow(R_upperarmX, 2.0) + Math.pow(R_upperarmY, 2.0))))
        val R_shoulder_angle = (180 * Math.acos(R_shoulder_angle_cos) / Math.PI)
        temp.add(R_shoulder_angle)
        if (temp.size < 3){
            dataset_rshoulder.add(R_shoulder_angle)
        }
        if (temp.size >= 5)  {
            dataset_rshoulder.add((temp [temp.size - 1] + temp[temp.size - 2] + temp[temp.size - 3] + temp[temp.size - 4] + temp[temp.size - 5])/5)
        }
        time.add(SystemClock.uptimeMillis())
        if (dataset_rshoulder.size >= 2)  {
            val a = (dataset_rshoulder [dataset_rshoulder.size - 1] - dataset_rshoulder[dataset_rshoulder.size - 2]) / (time[time.size -1] - time[time.size - 2])
            dataset_rshoulder_av.add(a*1000)
        }
        if(temp.size >= 101){
            temp.clear()
            dataset_rshoulder.clear()
            dataset_rshoulder_av.clear()
            time.clear()
        }
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawText(
            "Right shoulder angle ="+String.format("%.2f",(180 * Math.acos(R_shoulder_angle) / Math.PI))+" degs",
            300F,
            150F,
            mPaint2
        )
        if (dataset_rshoulder_av.size >=1) {
            canvas.drawText(
                "Right shoulder angular velocity ="+String.format("%.2f",dataset_rshoulder_av[dataset_rshoulder_av.size-1]) + " degs/s", 280F, 450F,
                mPaint2
            )
        }
        val paint = Paint()
        ...
        dataset_rshoulder.forEachIndexed {index, currentDataPoint ->

            if (index < dataset_rshoulder.size - 1) {
                val nextDataPoint = dataset_rshoulder[index + 1]
            ...
            dataset_rshoulder_av.forEachIndexed { index, currentDataPoint ->

                if (index < dataset_rshoulder_av.size - 1) {
                    val nextDataPoint = dataset_rshoulder_av[index + 1]
                    val startX = index.toFloat()+ XPoint
