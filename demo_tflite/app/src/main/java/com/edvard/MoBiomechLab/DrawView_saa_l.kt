...
class DrawView_saa_l : View {
    ...
    private var dataset_lshoulder = mutableListOf<Double>()
    private var dataset_lshoulder_av= mutableListOf<Double>()
    ...
    fun setDrawPoint(
        ...
        val L_trunkX = (mDrawPoint[8].x - mDrawPoint[2].x).toDouble()
        val L_upperarmX = (mDrawPoint[3].x - mDrawPoint[2].x).toDouble()
        val L_trunkY = (mDrawPoint[8].y - mDrawPoint[2].y).toDouble()
        val L_upperarmY = (mDrawPoint[3].y - mDrawPoint[2].y).toDouble()
        val L_elbow_angle_cos =
            ((L_trunkX * L_upperarmX + L_trunkY * L_upperarmY) / (Math.sqrt(
                Math.pow(
                    L_trunkX,
                    2.0
                ) + Math.pow(L_trunkY, 2.0)
            ) * Math.sqrt(Math.pow(L_upperarmX, 2.0) + Math.pow(L_upperarmY, 2.0))))
        val L_shoulder_angle = (180 * Math.acos(L_elbow_angle_cos) / Math.PI)
        temp.add(L_shoulder_angle)
        if (temp.size < 3){
            dataset_lshoulder.add(L_shoulder_angle)
        }
        if (temp.size >= 5)  {
            dataset_lshoulder.add((temp [temp.size - 1] + temp[temp.size - 2] + temp[temp.size - 3] + temp[temp.size - 4] + temp[temp.size - 5])/5)
        }
        time.add(SystemClock.uptimeMillis())
        if (dataset_lshoulder.size >= 2)  {
            val a = (dataset_lshoulder [dataset_lshoulder.size - 1] - dataset_lshoulder[dataset_lshoulder.size - 2]) / (time[time.size -1] - time[time.size - 2])
            dataset_lshoulder_av.add(a*1000)
        }
        if(temp.size >= 101){
            temp.clear()
            dataset_lshoulder.clear()
            dataset_lshoulder_av.clear()
            time.clear()
        }
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawText(
            "Left shoulder angle ="+String.format("%.2f",(180 * Math.acos(L_shoulder_angle) / Math.PI))+" degs",
            300F,
            150F,
            mPaint2
        )
        if (dataset_lshoulder_av.size >=1) {
            canvas.drawText(
                "Left shoulder angular velocity ="+String.format("%.2f",dataset_lshoulder_av[dataset_lshoulder_av.size-1]) + " degs/s", 280F, 450F,
                mPaint2
            )
        }
        val paint = Paint()
        ...
        dataset_lshoulder.forEachIndexed {index, currentDataPoint ->

            if (index < dataset_lshoulder.size - 1) {
                val nextDataPoint = dataset_lshoulder[index + 1]
            ...
            dataset_lshoulder_av.forEachIndexed { index, currentDataPoint ->

                if (index < dataset_lshoulder_av.size - 1) {
                    val nextDataPoint = dataset_lshoulder_av[index + 1]
                    val startX = index.toFloat()+ XPoint
